...
export class EgitimComponent implements OnInit {
  ...
  ngOnInit() {
    this.pageTitleService.setPage('Eğitim', 'VRF, Isı Pompası ve yazılım eğitim modülleri yönetimi');
    this.initRegionTree();
  }
  ...
  get availableCategories(): string[] {
    return this.faqCatService.getCategoriesByModule('Eğitim').map(c => c.name);
  }

  get pendingApprovalCount(): number {
    return this.faqService.getItemsByModule('Eğitim').filter(q => q.status === 'Pending_Approval').length;
  }

  get filteredLessons(): FaqItem[] {
    const user = this.authService.currentUser();
    const q = this.searchQuery.trim().toLowerCase();

    return this.faqService.getItemsByModule('Eğitim').filter(item => {
      if (this.authService.isCountryAdmin() && user.assignedCountry) {
        if (item.scope === 'Ülke') {
  ...
  openEditModal(item: FaqItem) {
    ...
    this.formContent = item.answerSnippet;
  ...
  resetForm() {
    ...
    this.formCategory = cats.length > 0 ? cats[0] : 'Saha Montaj Eğitimleri';
    this.formContent = '';
  ...
  saveLesson() {
    ...
    if (this.editingId) {
      this.faqService.updateQuestion(this.editingId, {
        ...
        answerSnippet: this.formContent.trim(),
  ...
  deleteLesson(id: number) {
    if (confirm('Bu eğitim konusunu silmek istediğinizden emin misiniz?')) {
