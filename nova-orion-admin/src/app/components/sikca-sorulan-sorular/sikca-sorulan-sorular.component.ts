...
@Component({
  selector: 'app-sikca-sorulan-sorular',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './sikca-sorulan-sorular.component.html',
  styleUrl: './sikca-sorulan-sorular.component.css'
})
export class SikcaSorulanSorularComponent implements OnInit {
  ...
  ngOnInit() {
    this.pageTitleService.setPage('Frequently Asked Questions', 'Kullanıcı yardım rehberleri, SSS soru ve yanıtları');
    this.initRegionTree();
  }
  ...
  toggleAllExpand() {
    const nextState = !this.regionTree.every(r => r.expanded);
    this.globalExpanded = nextState;
    this.regionTree.forEach(r => r.expanded = nextState);
  }

  getRegionSelectedCount(r: RegionNode): number {
    return r.countries.filter(c => c.checked).length;
  }
  ...
  initRegionTree(selectedCountries?: string[], scope?: ContentScope, targetRegion?: string, targetCountry?: string) {
    ...
    // If Country Admin, lock / select only assigned country
    ...
    // If Region Admin, restrict / select only their region's countries by default
  ...
  getCalculatedScope(): ContentScope {
    ...
    // Check if exactly one region has all countries checked and others none
  ...
  get availableCategories(): string[] {
    return this.faqCatService.getCategoriesByModule('SSS').map(c => c.name);
  }

  get totalViews(): number {
    return this.filteredQuestions.reduce((sum, q) => sum + q.viewCount, 0);
  }

  get publishedCount(): number {
    return this.filteredQuestions.filter(q => q.status === 'Published').length;
  }

  get pendingApprovalCount(): number {
    return this.faqService.getItemsByModule('SSS').filter(q => q.status === 'Pending_Approval').length;
  }

  get filteredQuestions(): FaqItem[] {
    const user = this.authService.currentUser();
    const q = this.searchQuery.trim().toLowerCase();

    return this.faqService.getItemsByModule('SSS').filter(item => {
  ...
  openEditModal(item: FaqItem) {
    ...
    this.formQuestion = item.question;
    this.formAnswer = item.answerSnippet;
  ...
  resetForm() {
    ...
    this.formCategory = cats.length > 0 ? cats[0] : 'Getting Started';
    this.formAnswer = '';
  ...
  insertFormat(type: string) {
    switch (type) {
      case 'bold': this.formAnswer += ' **kalın metin**'; break;
      case 'italic': this.formAnswer += ' *italik metin*'; break;
      case 'underline': this.formAnswer += ' <u>altı çizili</u>'; break;
      case 'link': this.formAnswer += ' [Bağlantı Başlığı](https://...)'; break;
      case 'image': this.formAnswer += ' ![Görsel](https://...)'; break;
      case 'code': this.formAnswer += ' `kod bloğu`'; break;
      case 'list': this.formAnswer += '\n• Liste öğesi 1\n• Liste öğesi 2'; break;
      case 'clear': this.formAnswer = ''; break;
    }
  }

  saveQuestion() {
    if (!this.formQuestion.trim() || !this.formAnswer.trim()) return;
    ...
    if (this.editingId) {
      this.faqService.updateQuestion(this.editingId, {
        ...
        answerSnippet: this.formAnswer.trim(),
